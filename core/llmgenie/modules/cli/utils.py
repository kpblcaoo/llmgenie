...
def load_config(root_dir: str) -> dict:
    """Load settings from core.llmgenie.toml or return empty dict."""
    import toml
    config_path = Path(root_dir) / "core.llmgenie.toml"
    if config_path.exists():
        try:
            with config_path.open("r", encoding="utf-8") as f:
                return toml.load(f)
        except Exception as e:
            logging.error(f"Failed to read core.llmgenie.toml: {e}")
    return {}
